...
from torchvision import transforms, models  # 加上 models
import torch.nn as nn                       # 加上 nn

def predict_single_image(image_path, model_path):
    # 1. 准备相同的特征提取器（空模型）
    model = models.resnet18()             # 构建一个空的 ResNet18 骨架（不需要传 weights，因为我们要加载自己的）
    num_features = model.fc.in_features
    model.fc = nn.Linear(num_features, 2) # 给它换上输出 2 个类别的分类头
...
if __name__ == '__main__':
    ...
    test_img = ".\my_dataset\cats\cats_0.jpg"
